f## Estimate the parameters from a 2 component guassian mixture model.
### Options for using either EM algorith or Newton's method.

## Make sure function is robust data from 1-component model (lambda = 1)
...
mu <- c(2, 0)
s2 <- c(1, 1)
y <- rnormmix(n, p, mu, sqrt(s2))
hist(y, breaks=50, col='gray', border='gray')


y <- read.table("Homework2/data/hw2_data.txt")
y <- y$V1


package.skeleton(name="Homework2", list=c("mixture"), force=TRUE)
